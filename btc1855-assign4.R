...
# Remove durations.hours.min column, create two new columns for both hours and minutes
...
# country column instead if it's blank 
ufo5 <- ufo4 %>%
  mutate(
...
ufo6 <- ufo5 %>%
  filter(!grepl("hoax|fake|prank|not real", tolower(comments)))
